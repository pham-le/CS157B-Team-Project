...
time = week number in year -> date
store = city -> store name
*/

SELECT t.date, t.day_number_in_month day_in_month, t.week_number_in_year week_in_year, p.description, p.brand brand, p.subcategory, s.name, s.store_street_address, s.city, s.store_state, sum(sf.unit_sales) total_unit_sold, sum(sf.customer_count) customer_count,
